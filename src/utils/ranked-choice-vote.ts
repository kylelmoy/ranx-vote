...
export function instantRunoff(votes: VoteResponse[]): InstantRunoffResult | null {
  ...
  while (true) {
    ...
    if (winner !== null) {
      rounds.push({
        voters,
        eliminated: Array.from(remainingCandidates).filter((c) => c !== winner),
      });
      break;
    }
    ...
    const lowest = Array.from(remainingCandidates).filter(
      (c) => (voters.get(c)?.length ?? 0) === min,
    );
